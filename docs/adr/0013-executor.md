...
- **Stato:** Accettata. La riga "SUCCEEDED → `complete_step`" di §5 passa per la verifica (ADR 0014 §4: `complete_step` solo dopo `EXECUTION_VERIFIED` passata; verifica fallita → `fail_step` e `fail`); tre precondizioni in più in §2 (ADR 0014 §3); la tabella di §8 estesa con le finestre 8a–8c e 9a (ADR 0014 §6); l'executor prende anche `verifiers: VerifierRegistryPort`. I quattro controlli sul percorso di §12 vivono in `ela.tools.paths.classify`, condivisa con il verifier, con l'ordine forma → risoluzione → link → bersaglio (ADR 0014 §2): stessi codici del tool, un link che punta fuori è `path.outside_workspace`. **ADR 0015**: il parametro `approval` di `execute` (§1, §3) è rimosso, la risposta arriva dall'`ApprovalStore`; l'`Approval` di §5 è persistita prima di `request_approval` e ha id `uuid5(APPROVAL_NAMESPACE, decision.id)`; l'`ExecutionResult` è persistito prima di `TOOL_EXECUTED`; la tabella di §8 è **sostituita** da ADR 0015 §8 (finestre 5, 7b, 8 riparate; 7a dichiarata); il vincolo "solo in-process" delle Conseguenze decade. **ADR 0018 §4 e ADR 0019 §4**: `execute(task_id, step_id, *, device_id)` — anche `arguments` sparisce (sono `step.arguments`) e il nodo scelto dall'orchestrator diventa un parametro obbligatorio; la costante `LOCAL_DEVICE` e il `device_id=None` sui risultati non esistono più.
- **Data:** 2026-09-06
