...
- **Stato:** Accettata. §9 (`complete`) esteso da ADR 0009 §7 con la guardia sugli step; le operazioni per step sono in ADR 0009 §5. §3: `PERMISSION_DECIDED` lo scrive il Guardian (`authorize`, ADR 0011 §8). `TOOL_EXECUTED` e `AUTHORIZATION_GRANTED` li scrive l'executor, che chiama `deny`, `request_approval`, `complete_step` e `fail_step` (ADR 0013 §5); il contratto import-linter 7 di §12 è a import diretti da ADR 0013 §13. §6 esteso da ADR 0015 §6: `recover()` fa scadere anche i task WAITING_APPROVAL la cui richiesta è scaduta (riga `expire`, attore `task-engine`, `RecoverySummary.expired`); l'engine prende `approvals: ApprovalStore`.
- **Data:** 2026-09-04
