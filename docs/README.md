...
(specifica, fonte di verità), `adr/` (Architecture Decision Records), `milestones/` (spec di ogni
milestone, e in `M9.4.md` l'elenco di ciò che v0.1 semplifica), `examples/` (i due piani di
esempio: quello che `GETTING_STARTED` usa e uno step `model.complete` per quando il provider avrà
una chiave — un test li manda all'API così come sono, perché non marciscano).
