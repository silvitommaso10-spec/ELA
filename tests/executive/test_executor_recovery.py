...
from tests.permissions.support import ECHO, ECHO_ARGS, GUARDED_ECHO, NOTE, NOTE_ARGS
...
async def test_window_5_a_stored_request_is_asked_again_not_duplicated() -> None:
    ...
    await w.engine.start(task.id)
    resumed = await w.executor.execute(task.id, step.id, NOTE_ARGS)
...
async def test_window_9a_a_step_failed_by_a_verification_fails_its_executing_task() -> None:
    ...
    with pytest.raises(SimulatedCrash):
        await w.executor.execute(task.id, first.id, ECHO_ARGS)
...
async def test_the_last_granted_request_of_the_step_is_the_one_that_grants(w: World) -> None:
    ...
    task, step = await w.running(NOTE.id, requires_authorization=True)
    first_ask = await w.executor.execute(task.id, step.id, NOTE_ARGS)
    ...
    second_ask = await w.executor.execute(task.id, step.id, NOTE_ARGS)
...
async def test_a_granted_request_of_another_step_is_not_seen(w: World) -> None:
    ...
    execution = await w.executor.execute(other_task.id, other_step.id, NOTE_ARGS)
...
async def test_the_approval_parameter_and_the_optional_stores_are_gone(w: World) -> None:
    task, step = await w.running(ECHO.id)
    with pytest.raises(TypeError):
        await w.executor.execute(task.id, step.id, ECHO_ARGS, approval=None)  # type: ignore[call-arg]
    with pytest.raises(TypeError):
...
async def test_nothing_of_the_user_enters_the_audit_on_a_retry_either() -> None:
    ...
    crashes.audit.arm("append", audit_of(E.TOOL_EXECUTED))
    with pytest.raises(SimulatedCrash):
        await w.executor.execute(task.id, step.id, SECRET_ARGS)
    crashes.disarm()
