...
- **Stato:** Accettata. §2-ter (limit=0) superata da ADR 0006 §11. `TaskRepository` estesa da ADR 0008 §2 (`add_plan`, `plan`). `CapabilityRegistryPort` ridotta a `get`, `specs` da ADR 0010 §1 (niente `register`). `PermissionGuardianPort.decide` estesa con il keyword-only `authorization_uses` da ADR 0011 §5; il Guardian di §4 è consegnato da ADR 0011. `AuthorizationStore` estesa con `consume` e ridotta senza `record_use` da ADR 0012 §3–4. `AuthorizingGuardianPort` (async, `authorize`) e `ToolRegistryPort` (sync, `get`, `tools`) introdotti da ADR 0013 §10: i port sono tredici.
- **Data:** 2026-09-04
