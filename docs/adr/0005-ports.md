...
- **Stato:** Accettata. §2-ter (limit=0) superata da ADR 0006 §11. `TaskRepository` estesa da ADR 0008 §2 (`add_plan`, `plan`). `CapabilityRegistryPort` ridotta a `get`, `specs` da ADR 0010 §1 (niente `register`). `PermissionGuardianPort.decide` estesa con il keyword-only `authorization_uses` da ADR 0011 §5; il Guardian di §4 è consegnato da ADR 0011.
- **Data:** 2026-09-04
